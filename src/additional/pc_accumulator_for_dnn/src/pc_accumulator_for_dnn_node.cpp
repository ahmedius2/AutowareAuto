...
{
	//Since ego_pose came, the corresponding tf should be available
	geometry_msgs::msg::TransformStamped::ConstSharedPtr tf;

	auto start = std::chrono::steady_clock::now();
  //NOTE this can take a long time, gotta check
	try {
    ...
    if(debug_mode_){
  		RCLCPP_INFO(this->get_logger(), "tdiff: %.1f ms", tdiff * 1000);
    }
		tf_res.header.stamp = ego_pose->header.stamp;
  ...
  if(pcloud_queue_.empty())
    return;

  auto most_recent_pc = pcloud_queue_.back();
  rclcpp::Time most_recent_pc_time = most_recent_pc->header.stamp;
  double tdiff = (most_recent_pc_time - tf_queue_.front()->header.stamp).seconds();
  ...
  if(pairs.empty() || (pairs.front().first->header.stamp != most_recent_pc_time)){
    ...
    num_all_points = most_recent_pc->width;
    pairs.clear();
